...
public class FileOperationsController {
    ...
    //REMOVEEEEEEEEEEE
@FXML
private void testStarvationPreventionHandler(ActionEvent event) {
    ...
    Task<Void> testTask = new Task<>() {
        ...
        protected Void call() throws Exception {
            ...
            RequestQueue requestQueue = loadBalancer.getRequestQueue();
            
            System.out.println("\n=== Starting Starvation Prevention Test ===");
            
            // Create a mix of high and low priority requests
            for (int i = 0; i < 10; i++) {
                // Create high priority requests first
                Request highPriorityRequest = new Request(
                    "HIGH-TEST-" + i,
                    i,
                    "UPLOAD",
                    "high-priority-user-" + i
                );
                requestQueue.addRequest(highPriorityRequest);
                
                // Create low priority requests that should be aged
                ...
                requestQueue.addRequest(lowPriorityRequest);
            ...
            System.out.println("\n=== Starvation Prevention Test Complete ===");
            return null;
        }
    };
    ...
    testTask.setOnSucceeded(e -> {
        Platform.runLater(() -> {
            ...
            showAlert(Alert.AlertType.INFORMATION, "Test Complete", 
                     "Starvation prevention test completed. Check console for results.");
        });
    });
    ...
    testTask.setOnFailed(e -> {
        Platform.runLater(() -> {
            ...
            showAlert(Alert.AlertType.ERROR, "Test Failed", 
                     "Test failed: " + testTask.getException().getMessage());
        });
    });
